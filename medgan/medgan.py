...
class Medgan(object):
    ...
    def buildGenerator(self, x_input, bn_train):
        ...
        with tf.variable_scope('generator', regularizer=l2_regularizer(self.l2scale)):
            ...
            for i, genDim in enumerate(self.generatorDims):
    ...
    def generateData(self,
        ...
        np.save('./generated/' + outFile, outputMat)
    ...
    def train(self,
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(nEpochs):
                ...
                self.print2file(buf, logFile)
                if epoch%10 == 0:
                    savePath = saver.save(sess, './models/'+outPath+'/'+outPath, global_step=epoch)
        print(savePath)
...
def parse_arguments(parser):
    ...
    parser.add_argument('--embed_size', type=int, default=16, help='The dimension size of the embedding, which will be generated by the generator. (default value: 128)')
    parser.add_argument('--noise_size', type=int, default=16, help='The dimension size of the random noise, on which the generator is conditioned. (default value: 128)')
    parser.add_argument('--generator_size', type=dims, default=(16, 16), help='The dimension size of the generator. Note that another layer of size "--embed_size" is always added. (default value: (128, 128))')
    parser.add_argument('--discriminator_size', type=dims, default=(32, 16, 1), help='The dimension size of the discriminator. (default value: (256, 128, 1))')
    parser.add_argument('--compressor_size', type=dims, default=(), help='The dimension size of the encoder of the autoencoder. Note that another layer of size "--embed_size" is always added. Therefore this can be a blank tuple. (default value: ())')
    parser.add_argument('--decompressor_size', type=dims, default=(), help='The dimension size of the decoder of the autoencoder. Note that another layer, whose size is equal to the dimension of the <patient_matrix>, is always added. Therefore this can be a blank tuple. (default value: ())')
    ...
    parser.add_argument('--pretrain_batch_size', type=int, default=10, help='The size of a single mini-batch for pre-training the autoencoder. (default value: 100)')
    parser.add_argument('--batch_size', type=int, default=50, help='The size of a single mini-batch for training medGAN. (default value: 1000)')
